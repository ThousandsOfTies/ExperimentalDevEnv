...
**「実機がなくてもクラウド上でハードウェア込みのテストができ、実機と同じバイナリがそのまま動く」**
...
| SSD1306 OLED | I2C | ⏳ モジュール到着待ち |

`sensor_demo` を実機で動かす最終検証は OLED 到着後に実施予定。
...
- [ ] OLED モジュール到着後、`sensor_demo` を RasPi5 実機で完全動作確認
- [ ] LCD HAT (ST7789) の対応（追加するなら）
- [ ] systemd サービス化による自動起動
- [ ] A/B シンボリックリンクによる無停止デプロイ機構
